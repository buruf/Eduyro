// This script replaces the Puppeteer/Chromium PDF renderer with @react-pdf/renderer
// which works natively on Vercel serverless without any browser dependency

const fs = require('fs');

// Step 1: Update renderer.ts to use @react-pdf/renderer
const rendererContent = `// src/lib/pdf/renderer.ts
...
fs.writeFileSync('src/lib/pdf/renderer.ts', rendererContent);
console.log('✅ renderer.ts rewritten with @react-pdf/renderer');

// Step 2: Update pack-cache.ts to use renderPackToPdf instead of renderHtmlToPdf
let cache = fs.readFileSync('src/lib/shop/pack-cache.ts', 'utf8');
cache = cache.replace(
  'import { renderHtmlToPdf } from "@/lib/pdf/renderer";',
  'import { renderPackToPdf } from "@/lib/pdf/renderer";'
);
cache = cache.replace(
  /const html = renderPackHtml\(([^)]+)\);\s*const pdfBytes = await renderHtmlToPdf\(html\);/g,
  'const pdfInput = { skillLabel: pack.label, skillCode: pack.skill, levelCode: pack.skill, sheets: mappedSheets };\n  const pdfBytes = await renderPackToPdf(pdfInput);'
);
// Fix sample PDF generation too
cache = cache.replace(
  /const sampleHtml = renderPackHtml\(([^)]+)\);\s*const samplePdfBytes = await renderHtmlToPdf\(sampleHtml\);/g,
...
fs.writeFileSync('src/lib/shop/pack-cache.ts', cache);
console.log('✅ pack-cache.ts updated to use renderPackToPdf');

console.log('\nDone. Now run:');
console.log('npm uninstall @sparticuz/chromium puppeteer-core');
