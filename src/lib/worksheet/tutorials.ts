...
function getMathTutorial(skill: string, skillName: string): TutorialContent {
  ...
  if (skill.includes("x2") || skill.includes("x3") || skill.includes("x4") || skill.includes("x5") || skill.includes("x2") || skill.includes("x5") || (skill.includes("multiplication") && !skill.includes("polynomial"))) return multiplicationTutorial();
...
function numberPatternsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Geometric Pattern", formula: "Multiply or divide by the same number each time", explanation: "If you multiply by the same number each step, it is a geometric pattern.", tip: "3, 6, 12, 24 -> rule is x2" },
    ...
    examples: [
      ...
      { problem: "1, 3, 9, 27, ___", steps: ["Each number is multiplied by 3.", "27 x 3 = 81"], answer: "81" },
      { problem: "What is the rule? 3, 6, 12, 24", steps: ["3 x 2 = 6", "6 x 2 = 12", "12 x 2 = 24", "The rule is: multiply by 2."], answer: "x2 (double each time)" },
    ],
  };
}
...
function numberBondsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Part + Part = Whole", formula: "Part_1 + Part_2 = Whole", explanation: "Every whole number can be split into two smaller parts.", tip: "8 = 3+5 = 2+6 = 4+4" },
    ...
    examples: [
      ...
      { problem: "8 + ___ = 15", steps: ["15 - 8 = 7", "Check: 8 + 7 = 15 OK"], answer: "7" },
...
function additionTutorial(max: number): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Check by Swapping", formula: "a + b = b + a  (commutative law)", explanation: "The order doesn't matter in addition. Swap the numbers and re-add to verify your answer.", tip: "46 + 27 = 27 + 46 = 73 OK" },
    ],
    examples: [
...
function missingNumbersTutorial(): TutorialContent {
  return {
    ...
    examples: [
      { problem: "___ + 8 = 13", steps: ["To find the missing number, subtract: 13 - 8 = 5", "Check: 5 + 8 = 13 OK"], answer: "5" },
      { problem: "25 - ___ = 11", steps: ["To find the missing number: 25 - 11 = 14", "Check: 25 - 14 = 11 OK"], answer: "14" },
      { problem: "___ x 6 = 42", steps: ["To find the missing number, divide: 42 / 6 = 7", "Check: 7 x 6 = 42 OK"], answer: "7" },
      { problem: "48 / ___ = 8", steps: ["To find the missing number: 48 / 8 = 6", "Check: 48 / 6 = 8 OK"], answer: "6" },
      { problem: "___ - 17 = 25", steps: ["To find the missing number, add: 25 + 17 = 42", "Check: 42 - 17 = 25 OK"], answer: "42" },
    ],
  };
}
...
function subtractionTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Check with Addition", formula: "answer + bottom = top", explanation: "Add your answer back to the number you subtracted. You should get the original top number.", tip: "82-45=37 -> check: 37+45=82 OK" },
    ],
    examples: [
...
function multiplicationTutorial(): TutorialContent {
  return {
    ...
    intro: "Multiplication is repeated addition. 3 x 4 means 3 groups of 4.",
    concepts: [
      { title: "What Multiplication Means", formula: "a x b = a groups of b added together", explanation: "3 x 4 means three groups of four: 4+4+4=12. The x symbol means groups of.", tip: "6 x 7 = 6 groups of 7 = 42" },
      { title: "Commutative Law", formula: "a x b = b x a", explanation: "The order does not change the answer. 6x7 = 7x6 = 42. This halves the facts you need to memorise!", tip: "If you know 8x3, you know 3x8!" },
      { title: "x9 Finger Trick", formula: "Hold up 10 fingers. Fold down the nth finger. Left = tens, Right = ones.", explanation: "For 9x6: fold down 6th finger. 5 fingers left = 5 tens, 4 right = 4 ones -> 54.", tip: "Try 9x7: fold 7th finger -> 6 left, 3 right = 63" },
    ],
    examples: [
      { problem: "6 x 7 = ?", steps: ["Think: 6 groups of 7", "6 x 7 = 42", "Memory trick: 6 sevens = 42"], answer: "42" },
      { problem: "8 x 9 = ?", steps: ["8 x 9 = 72", "Memory trick: 9 x 8 = 72 (9s trick: 7+2=9)"], answer: "72" },
      { problem: "12 x 6 = ?", steps: ["Split 12 into 10 + 2", "10 x 6 = 60", "2 x 6 = 12", "60 + 12 = 72"], answer: "72" },
      { problem: "7 x ___ = 56", steps: ["We need: how many 7s make 56?", "56 / 7 = 8", "Check: 7 x 8 = 56 OK"], answer: "8" },
      { problem: "25 x 4 = ?", steps: ["25 x 4 = 25 + 25 + 25 + 25", "Or: 25 x 4 = 100 (quarter x 4 = whole)"], answer: "100" },
    ],
  };
}
...
function divisionTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Division <-> Multiplication", formula: "If a x b = c, then c / a = b", explanation: "Division and multiplication are inverses. Use your times tables to solve division problems.", tip: "63 / 7: ask 7 x ? = 63 -> answer is 9" },
    ],
    examples: [
      { problem: "42 / 6 = ?", steps: ["Ask: how many 6s fit into 42?", "6 x 7 = 42", "So 42 / 6 = 7"], answer: "7" },
      { problem: "63 / 9 = ?", steps: ["Ask: 9 x ? = 63", "9 x 7 = 63", "So 63 / 9 = 7"], answer: "7" },
      { problem: "72 / 8 = ?", steps: ["8 x 9 = 72", "So 72 / 8 = 9"], answer: "9" },
      { problem: "132 / 11 = ?", steps: ["11 x 10 = 110", "11 x 12 = 132", "So 132 / 11 = 12"], answer: "12" },
      { problem: "156 / 12 = ?", steps: ["12 x 10 = 120", "156 - 120 = 36", "12 x 3 = 36", "So 12 x 13 = 156", "156 / 12 = 13"], answer: "13" },
    ],
  };
}
...
function divisionRemaindersTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "What is a Remainder?", formula: "dividend = (divisor x quotient) + remainder", explanation: "When a number does not divide evenly, what is left over is the remainder. It is always LESS than the divisor.", tip: "17 / 5 = 3 R 2 -> 5x3=15 and 17-15=2" },
      { title: "Find the Largest Multiple", formula: "Find biggest multiple of divisor that fits inside dividend", explanation: "Step 1: Find the largest multiple of the divisor that does not exceed the dividend. Step 2: Subtract to get the remainder.", tip: "23 / 4: 4x5=20 fits, 23-20=3. Answer: 5 R 3" },
    ],
    examples: [
      { problem: "17 / 5 = ?", steps: ["5 x 3 = 15 (closest without going over)", "17 - 15 = 2 left over", "Answer: 3 remainder 2, or 3 R 2"], answer: "3 R 2" },
      { problem: "23 / 4 = ?", steps: ["4 x 5 = 20 (closest without going over)", "23 - 20 = 3 left over", "Answer: 5 R 3"], answer: "5 R 3" },
      { problem: "37 / 6 = ?", steps: ["6 x 6 = 36", "37 - 36 = 1 left over", "Answer: 6 R 1"], answer: "6 R 1" },
      { problem: "50 / 7 = ?", steps: ["7 x 7 = 49", "50 - 49 = 1 left over", "Answer: 7 R 1"], answer: "7 R 1" },
      { problem: "100 / 9 = ?", steps: ["9 x 11 = 99", "100 - 99 = 1 left over", "Answer: 11 R 1"], answer: "11 R 1" },
    ],
  };
}
...
function fractionIdentificationTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Equivalent Fractions", formula: "1/2 = 2/4 = 3/6 = 4/8", explanation: "Multiply or divide BOTH top and bottom by the same number to get an equivalent fraction.", tip: "1/2 x 2/2 = 2/4  -- same amount, different form" },
    ],
    examples: [
...
function fractionComparisonTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Cross Multiplication", formula: "a/b vs c/d: compare axd with cxb", explanation: "Multiply diagonally. The fraction with the bigger cross-product is greater.", tip: "2/3 vs 3/5: 2x5=10 vs 3x3=9 -> 2/3 wins" },
    ],
    examples: [
...
function decimalPlaceValueTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Place Value Chart", formula: "Hundreds | Tens | Ones . Tenths | Hundredths | Thousandths", explanation: "The decimal point separates whole numbers from fractions. Each place is 10x smaller than the one to its left.", tip: "In 3.47 -> 3=ones, 4=tenths, 7=hundredths" },
...
function decimalOperationsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Multiply by 10: Shift Right", formula: "n x 10 -> move decimal RIGHT by 1 place", explanation: "Multiplying by 10, 100, 1000 shifts the decimal point right by 1, 2, 3 places.", tip: "0.46 x 10 = 4.6    0.46 x 100 = 46" },
    ...
    examples: [
      ...
      { problem: "0.5 x 10 = ?", steps: ["When multiplying by 10, move decimal right 1 place.", "0.5 x 10 = 5.0 = 5"], answer: "5" },
...
function percentagesTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Finding a Percentage", formula: "% of number = (% / 100) x number", explanation: "Convert the percentage to a decimal then multiply.", tip: "15% of 60 = 0.15 x 60 = 9" },
    ...
    examples: [
      { problem: "What is 10% of 80?", steps: ["10% = 10/100 = 0.1", "0.1 x 80 = 8", "Or: 80 / 10 = 8"], answer: "8" },
      ...
      { problem: "What percentage is 30 out of 120?", steps: ["Percentage = (part / whole) x 100", "30 / 120 = 0.25", "0.25 x 100 = 25%"], answer: "25%" },
    ],
  };
}
...
function ratiosTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Equivalent Ratios", formula: "a:b = (axn):(bxn)", explanation: "Multiply both parts by the same number to get an equivalent ratio.", tip: "2:3 = 4:6 = 6:9" },
    ],
    examples: [
      ...
      { problem: "The ratio 2:3. If the first quantity is 8, what is the second?", steps: ["2:3 = 8:?", "Multiply both by 4: 2x4=8, 3x4=12", "Answer: 12"], answer: "12" },
      ...
      { problem: "A recipe uses 3 cups flour to 2 cups sugar. For 9 cups flour, how much sugar?", steps: ["Ratio flour:sugar = 3:2", "9 cups flour = 3 x 3", "Sugar = 2 x 3 = 6 cups"], answer: "6 cups" },
    ],
  };
}
...
function proportionsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Cross Multiplication", formula: "a/b = c/d  ->  axd = bxc", explanation: "Multiply diagonally. The products must be equal. Use this to solve for an unknown variable.", tip: "3/x = 9/15: 3x15=9xx -> 45=9x -> x=5" },
    ...
    examples: [
      { problem: "Solve: x/4 = 3/12", steps: ["Cross multiply: 12x = 4 x 3 = 12", "x = 12 / 12 = 1"], answer: "x = 1" },
      { problem: "If 5 apples cost $2.50, how much do 15 apples cost?", steps: ["Set up proportion: 5/2.50 = 15/x", "Cross multiply: 5x = 2.50 x 15 = 37.50", "x = 37.50 / 5 = 7.50"], answer: "$7.50" },
      { problem: "Solve: 3/8 = 9/x", steps: ["Cross multiply: 3x = 8 x 9 = 72", "x = 72 / 3 = 24"], answer: "x = 24" },
      { problem: "A car travels 150 km in 2 hours. How far in 5 hours?", steps: ["150/2 = x/5", "Cross multiply: 2x = 150 x 5 = 750", "x = 375 km"], answer: "375 km" },
      { problem: "Solve: 2/5 = x/20", steps: ["Cross multiply: 5x = 2 x 20 = 40", "x = 40 / 5 = 8"], answer: "x = 8" },
    ],
  };
}
...
function unitRatesTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "The 'Per' Word", formula: "'per' always means /", explanation: "When you see 'per', divide. To find total: multiply unit rate x quantity.", tip: "$1.50/kg x 4kg = $6 total" },
...
function oneStepEquationsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Inverse Operations", formula: "+<->-    x<->/", explanation: "To cancel an operation, use its inverse. To remove +7, subtract 7.", tip: "You are trying to get x ALONE on one side" },
      { title: "Always Check!", formula: "Substitute answer back into original equation", explanation: "After solving, replace x with your answer. Both sides should be equal.", tip: "x=7 in x+5=12: 7+5=12 OK" },
    ],
    examples: [
      { problem: "Solve: x + 7 = 12", steps: ["Subtract 7 from both sides:", "x + 7 - 7 = 12 - 7", "x = 5", "Check: 5 + 7 = 12 OK"], answer: "x = 5" },
      { problem: "Solve: x - 4 = 9", steps: ["Add 4 to both sides:", "x - 4 + 4 = 9 + 4", "x = 13"], answer: "x = 13" },
      { problem: "Solve: 3x = 21", steps: ["Divide both sides by 3:", "3x / 3 = 21 / 3", "x = 7", "Check: 3 x 7 = 21 OK"], answer: "x = 7" },
      { problem: "Solve: x/5 = 6", steps: ["Multiply both sides by 5:", "x/5 x 5 = 6 x 5", "x = 30"], answer: "x = 30" },
...
function twoStepEquationsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Reverse PEMDAS", formula: "Undo + and - FIRST, then x and /", explanation: "When solving, reverse the order of operations: undo addition/subtraction first, then multiplication/division.", tip: "2x+3=11: undo +3 first -> 2x=8, then /2 -> x=4" },
      { title: "Two-Step Strategy", formula: "Step 1: +/-   Step 2: x/", explanation: "Move constants to the other side first. Then divide by the coefficient of x.", tip: "3x-5=16 -> +5 -> 3x=21 -> /3 -> x=7" },
    ],
    examples: [
      { problem: "Solve: 2x + 3 = 11", steps: ["Step 1: Subtract 3 from both sides: 2x = 8", "Step 2: Divide both sides by 2: x = 4", "Check: 2(4) + 3 = 11 OK"], answer: "x = 4" },
      { problem: "Solve: 3x - 5 = 16", steps: ["Step 1: Add 5 to both sides: 3x = 21", "Step 2: Divide by 3: x = 7", "Check: 3(7) - 5 = 16 OK"], answer: "x = 7" },
      { problem: "Solve: 4x + 8 = 28", steps: ["Step 1: 4x = 28 - 8 = 20", "Step 2: x = 20 / 4 = 5"], answer: "x = 5" },
      { problem: "Solve: x/3 + 4 = 9", steps: ["Step 1: x/3 = 9 - 4 = 5", "Step 2: x = 5 x 3 = 15"], answer: "x = 15" },
...
function inequalitiesTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "FLIP THE SIGN", formula: "When x/ by a NEGATIVE number: flip the inequality!", explanation: "This is the critical exception when solving inequalities.", tip: "-2x > 6: divide by -2 and FLIP -> x < -3" },
    ],
    examples: [
...
function wordProblemsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Key Words -> Operations", formula: "total/altogether -> +    left/fewer -> -    groups of -> x    each/share -> /", explanation: "Certain words signal which operation to use.", tip: "'How many MORE' -> subtraction!" },
    ...
    examples: [
      ...
      { problem: "A shirt costs $35. There is a 20% discount. What is the sale price?", steps: ["Discount = 20% of $35 = 0.2 x 35 = $7", "Sale price = $35 - $7 = $28"], answer: "$28" },
      { problem: "A rectangle is 12 cm long and 7 cm wide. What is its area?", steps: ["Area = length x width", "Area = 12 x 7 = 84 cm^2"], answer: "84 cm^2" },
...
function slopeInterceptTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "What is Slope?", formula: "m = rise / run = (y_2-y_1)/(x_2-x_1)", explanation: "Slope measures steepness: units up per unit right. Positive = upward. Negative = downward.", tip: "Slope 3 means: right 1, up 3" },
...
function graphingLinesTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Parallel and Perpendicular", formula: "Parallel: same slope    Perpendicular: slopes multiply to -1", explanation: "Parallel lines never meet (same m). Perpendicular lines cross at 90 degrees (m_1xm_2=-1).", tip: "Perpendicular to slope 3: slope = -1/3" },
    ],
    examples: [
...
function addingPolynomialsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Like Terms", formula: "Same variable AND same exponent = like terms", explanation: "You can ONLY combine terms that have exactly the same variable raised to the same power.", tip: "3x and 7x are like    3x and 3x^2 are NOT like" },
      { title: "Combining Like Terms", formula: "Add/subtract the COEFFICIENTS, keep the variable", explanation: "Only the number in front (coefficient) changes. The variable and exponent stay exactly the same.", tip: "3x^2 + 5x^2 = 8x^2 (only the 3+5 changes)" },
      { title: "Standard Form", formula: "Arrange terms: x^3 -> x^2 -> x -> constant (highest to lowest power)", explanation: "After combining, put terms in decreasing order of exponent.", tip: "(2x^2+x)+(x+3) = 2x^2+2x+3" },
    ],
    examples: [
      { problem: "(3x + 5) + (2x + 4) = ?", steps: ["Group like terms: (3x + 2x) + (5 + 4)", "= 5x + 9"], answer: "5x + 9" },
      { problem: "(x^2 + 3x) + (2x^2 + x) = ?", steps: ["x^2 terms: x^2 + 2x^2 = 3x^2", "x terms: 3x + x = 4x", "Answer: 3x^2 + 4x"], answer: "3x^2 + 4x" },
      { problem: "(4x^2 - 2x + 1) + (x^2 + 5x - 3) = ?", steps: ["x^2 terms: 4x^2 + x^2 = 5x^2", "x terms: -2x + 5x = 3x", "Constant: 1 + (-3) = -2", "Answer: 5x^2 + 3x - 2"], answer: "5x^2 + 3x - 2" },
      { problem: "(7x - 3) + (-4x + 8) = ?", steps: ["x terms: 7x + (-4x) = 3x", "Constants: -3 + 8 = 5", "Answer: 3x + 5"], answer: "3x + 5" },
      { problem: "(2x^3 + x) + (x^3 - 3x + 4) = ?", steps: ["x^3 terms: 2x^3 + x^3 = 3x^3", "x terms: x + (-3x) = -2x", "Constants: 0 + 4 = 4", "Answer: 3x^3 - 2x + 4"], answer: "3x^3 - 2x + 4" },
    ],
  };
}
...
function multiplyingPolynomialsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "FOIL Method", formula: "(a+b)(c+d) = First + Outer + Inner + Last", explanation: "Multiply each term in the first bracket by each term in the second. FOIL is the memory order.", tip: "(x+2)(x+3): F=x^2, O=3x, I=2x, L=6 -> x^2+5x+6" },
      { title: "Difference of Squares", formula: "(a+b)(a-b) = a^2 - b^2", explanation: "When you multiply a sum and difference with the same terms, the middle terms cancel out perfectly.", tip: "(x+5)(x-5) = x^2-25 (no middle term!)" },
      { title: "Perfect Square", formula: "(a+b)^2 = a^2 + 2ab + b^2", explanation: "Squaring a binomial: square the first, double the product, square the last.", tip: "(x+3)^2 = x^2+6x+9  (middle = 2xxx3)" },
    ],
    examples: [
      { problem: "(x + 2)(x + 3) = ?", steps: ["FOIL:", "First: x x x = x^2", "Outer: x x 3 = 3x", "Inner: 2 x x = 2x", "Last: 2 x 3 = 6", "Combine: x^2 + 3x + 2x + 6 = x^2 + 5x + 6"], answer: "x^2 + 5x + 6" },
      { problem: "(x + 4)(x - 1) = ?", steps: ["First: x^2", "Outer: -x", "Inner: 4x", "Last: -4", "Combine: x^2 + 3x - 4"], answer: "x^2 + 3x - 4" },
      { problem: "(x - 3)(x - 2) = ?", steps: ["First: x^2", "Outer: -2x", "Inner: -3x", "Last: +6", "Combine: x^2 - 5x + 6"], answer: "x^2 - 5x + 6" },
      { problem: "(2x + 1)(x + 3) = ?", steps: ["First: 2x^2", "Outer: 6x", "Inner: x", "Last: 3", "Combine: 2x^2 + 7x + 3"], answer: "2x^2 + 7x + 3" },
      { problem: "(x + 5)(x - 5) = ?", steps: ["This is difference of squares: (a+b)(a-b) = a^2 - b^2", "= x^2 - 25"], answer: "x^2 - 25" },
    ],
  };
}
...
function factoringTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Factoring x^2 + bx + c", formula: "Find two numbers: multiply to c AND add to b", explanation: "Search for a factor pair of c whose sum equals b. These become the constants in your two brackets.", tip: "x^2+5x+6: 2x3=6 and 2+3=5 -> (x+2)(x+3)" },
      { title: "Difference of Squares", formula: "a^2 - b^2 = (a+b)(a-b)", explanation: "Two perfect squares with a minus between them factor instantly with this pattern.", tip: "x^2-16 = (x+4)(x-4)    4x^2-9 = (2x+3)(2x-3)" },
    ],
    examples: [
      { problem: "Factor: x^2 + 5x + 6", steps: ["Find two numbers that multiply to 6 and add to 5.", "2 x 3 = 6 and 2 + 3 = 5 OK", "Answer: (x + 2)(x + 3)"], answer: "(x + 2)(x + 3)" },
      { problem: "Factor: x^2 - 9 (difference of squares)", steps: ["x^2 - 9 = x^2 - 3^2", "a^2 - b^2 = (a+b)(a-b)", "= (x + 3)(x - 3)"], answer: "(x + 3)(x - 3)" },
      { problem: "Factor: x^2 + 7x + 12", steps: ["Find two numbers: multiply to 12, add to 7", "3 x 4 = 12, 3 + 4 = 7 OK", "Answer: (x + 3)(x + 4)"], answer: "(x + 3)(x + 4)" },
      { problem: "Factor out the GCF: 6x + 9", steps: ["GCF of 6 and 9 = 3", "3(2x + 3)"], answer: "3(2x + 3)" },
      { problem: "Factor: x^2 - x - 6", steps: ["Find two numbers: multiply to -6, add to -1", "-3 x 2 = -6, -3 + 2 = -1 OK", "Answer: (x - 3)(x + 2)"], answer: "(x - 3)(x + 2)" },
    ],
  };
}
...
function quadraticEquationsTutorial(): TutorialContent {
  return {
    ...
    intro: "Quadratic equations have x^2. Solve by factoring: set each factor equal to zero.",
    concepts: [
      { title: "Standard Form", formula: "ax^2 + bx + c = 0  (everything on one side = 0)", explanation: "Always rearrange first. The equation must equal zero before you can use factoring or the formula.", tip: "x^2+3x=4 -> x^2+3x-4=0 (standard form)" },
      { title: "Zero Product Property", formula: "If A x B = 0, then A=0 OR B=0", explanation: "If two things multiply to zero, at least one must be zero. This is how factoring leads to solutions.", tip: "(x-3)(x+2)=0 -> x=3 or x=-2" },
      { title: "Methods", formula: "1. Factoring   2. Quadratic formula   3. Square root (when b=0)", explanation: "Choose the fastest method. Factoring works when numbers are nice. Formula always works.", tip: "x^2=16 -> x=+/-4 (square root method)" },
    ],
    examples: [
      { problem: "Solve: x^2 + 5x + 6 = 0", steps: ["Factor: (x + 2)(x + 3) = 0", "Set each factor to zero:", "x + 2 = 0 -> x = -2", "x + 3 = 0 -> x = -3", "Solutions: x = -2 or x = -3"], answer: "x = -2 or x = -3" },
      { problem: "Solve: x^2 - 7x + 12 = 0", steps: ["Factor: (x - 3)(x - 4) = 0", "x = 3 or x = 4"], answer: "x = 3 or x = 4" },
      { problem: "Solve: x^2 - 9 = 0", steps: ["x^2 = 9", "x = +/-sqrt9 = +/-3", "x = 3 or x = -3"], answer: "x = 3 or x = -3" },
      { problem: "Solve: x^2 + 4x = 0", steps: ["Factor out x: x(x + 4) = 0", "x = 0 or x + 4 = 0", "x = 0 or x = -4"], answer: "x = 0 or x = -4" },
      { problem: "Solve: 2x^2 - 8 = 0", steps: ["2x^2 = 8", "x^2 = 4", "x = +/-2"], answer: "x = 2 or x = -2" },
    ],
  };
}
...
function quadraticFormulaTutorial(): TutorialContent {
  return {
    ...
    intro: "For ax^2 + bx + c = 0, use x = (-b +/- sqrt(b^2-4ac)) / 2a. The discriminant b^2-4ac tells you how many solutions.",
    concepts: [
      { title: "The Quadratic Formula", formula: "x = (-b +/- sqrt(b^2-4ac)) / 2a", explanation: "This formula gives exact solutions for any quadratic equation -- it always works. Memorise it!", tip: "Negative b, plus or minus root, b squared minus 4ac, all over 2a" },
      { title: "The Discriminant", formula: "delta = b^2-4ac    delta>0: two solutions    delta=0: one    delta<0: none", explanation: "Check the discriminant FIRST -- it tells you how many real solutions exist before you do any work.", tip: "delta=0 means a perfect square (repeated root)" },
      { title: "Identify a, b, c First", formula: "ax^2+bx+c=0  ->  write down a, b, c before substituting", explanation: "Clearly identify all three coefficients before substituting into the formula.", tip: "2x^2-3x+1=0: a=2, b=-3, c=1" },
    ],
    examples: [
      { problem: "Solve x^2 + 4x + 4 = 0 using the quadratic formula", steps: ["a=1, b=4, c=4", "Discriminant: 4^2 - 4(1)(4) = 16 - 16 = 0", "x = -4 / 2 = -2 (one solution)"], answer: "x = -2" },
      { problem: "Solve x^2 - 5x + 6 = 0", steps: ["a=1, b=-5, c=6", "Discriminant: 25 - 24 = 1", "x = (5 +/- 1) / 2", "x = 3 or x = 2"], answer: "x = 3 or x = 2" },
      { problem: "How many solutions does x^2 + x + 1 = 0 have?", steps: ["Discriminant: 1^2 - 4(1)(1) = 1 - 4 = -3", "Discriminant < 0 -> no real solutions"], answer: "No real solutions" },
      { problem: "What is the discriminant of 2x^2 + 3x - 2 = 0?", steps: ["a=2, b=3, c=-2", "b^2 - 4ac = 9 - 4(2)(-2) = 9 + 16 = 25"], answer: "25 (two real solutions)" },
      { problem: "Solve x^2 - 4 = 0 using the quadratic formula", steps: ["a=1, b=0, c=-4", "x = (0 +/- sqrt16) / 2 = +/-4/2 = +/-2"], answer: "x = 2 or x = -2" },
    ],
  };
}
...
function parabolasTutorial(): TutorialContent {
  return {
    ...
    intro: "The parabola y = a(x-h)^2 + k has vertex (h,k). If a>0 it opens up; if a<0 it opens down.",
    concepts: [
      { title: "Vertex Form", formula: "y = a(x-h)^2 + k   ->   vertex = (h, k)", explanation: "Vertex form shows the vertex directly. h is x of the turning point, k is y.", tip: "y=(x-3)^2+2: vertex=(3,2)" },
      { title: "Opens Up or Down?", formula: "a > 0: opens UP (smiley union)    a < 0: opens DOWN (frowny intersect)", explanation: "The sign of 'a' determines which way the parabola opens.", tip: "y=-2x^2 opens downward (a=-2 < 0)" },
      { title: "Axis of Symmetry", formula: "x = h  (vertical line through the vertex)", explanation: "Every parabola is perfectly symmetric about the vertical line x=h.", tip: "y=(x-4)^2+1: axis of symmetry is x=4" },
    ],
    examples: [
      { problem: "Find the vertex of y = (x - 3)^2 + 2", steps: ["In y = a(x-h)^2 + k, h=3, k=2", "Vertex = (3, 2)"], answer: "Vertex: (3, 2)" },
      { problem: "Does y = -2(x+1)^2 + 5 open up or down?", steps: ["a = -2 (negative)", "Negative a -> opens downward"], answer: "Downward" },
      { problem: "Find the vertex of y = x^2 - 6x + 8", steps: ["Complete the square or use h = -b/2a", "h = 6/2 = 3", "k = 3^2 - 6(3) + 8 = 9 - 18 + 8 = -1", "Vertex = (3, -1)"], answer: "Vertex: (3, -1)" },
      { problem: "What is the axis of symmetry of y = (x - 4)^2 + 1?", steps: ["Axis of symmetry passes through vertex x-coordinate.", "x = 4"], answer: "x = 4" },
      { problem: "Find the y-intercept of y = x^2 - 4x + 3", steps: ["Set x = 0: y = 0 - 0 + 3 = 3", "y-intercept = (0, 3)"], answer: "(0, 3)" },
    ],
  };
}
...
function functionNotationTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Input -> Output", formula: "x (input) -> function machine -> f(x) (output)", explanation: "Same input always gives same output. Think of it as a machine.", tip: "f(x)=x^2: f(4)=16 and f(-4)=16 -- same output for different inputs!" },
    ...
    examples: [
      ...
      { problem: "If g(x) = x^2 - 1, find g(3)", steps: ["g(3) = 3^2 - 1 = 9 - 1 = 8"], answer: "8" },
      { problem: "If h(x) = 3x - 7, find h(0)", steps: ["h(0) = 3(0) - 7 = -7"], answer: "-7" },
      { problem: "If f(x) = x^2 + 2x, find f(-2)", steps: ["f(-2) = (-2)^2 + 2(-2) = 4 - 4 = 0"], answer: "0" },
...
function domainRangeTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Range = Possible Outputs", formula: "range = all y-values f(x) can actually produce", explanation: "The range is every y-value the function can output. Some functions can never produce certain values.", tip: "f(x)=x^2: output always >=0 -> range: y>=0" },
      { title: "Common Restrictions", formula: "No / by 0    No sqrt(negative)    log(positive only)", explanation: "Three main domain restrictions to check for.", tip: "f(x)=sqrt(x-4): need x-4>=0 -> domain: x>=4" },
    ],
    examples: [
      ...
      { problem: "What is the domain of f(x) = sqrtx?", steps: ["Square root of a negative number is not real.", "x must be >= 0", "Domain: x >= 0"], answer: "x >= 0" },
      { problem: "What is the range of f(x) = x^2?", steps: ["x^2 is always >= 0 for any real x", "Range: y >= 0"], answer: "y >= 0" },
      { problem: "What is the domain of f(x) = sqrt(x - 4)?", steps: ["x - 4 >= 0", "x >= 4"], answer: "x >= 4" },
...
function inverseFunctionsTutorial(): TutorialContent {
  return {
    ...
    intro: "The inverse function f^-^1 reverses what f does. To find it: swap x and y, then solve for y.",
    concepts: [
      { title: "What is an Inverse?", formula: "f^-^1(f(x)) = x    and    f(f^-^1(x)) = x", explanation: "The inverse completely undoes the original. If f adds 3, f^-^1 subtracts 3.", tip: "f(x)=x+5: f^-^1(x)=x-5" },
      ...
      { title: "Graphical Meaning", formula: "f and f^-^1 are mirror images across y = x", explanation: "Every point (a,b) on f becomes (b,a) on f^-^1.", tip: "If (2,7) is on f, then (7,2) is on f^-^1" },
    ],
    examples: [
      { problem: "Find the inverse of f(x) = 2x + 4", steps: ["Write y = 2x + 4", "Swap x and y: x = 2y + 4", "Solve for y: 2y = x - 4, y = (x-4)/2", "f^-^1(x) = (x-4)/2"], answer: "f^-^1(x) = (x-4)/2" },
      { problem: "Find the inverse of f(x) = 3x", steps: ["y = 3x -> swap -> x = 3y", "y = x/3", "f^-^1(x) = x/3"], answer: "f^-^1(x) = x/3" },
      { problem: "Verify: if f(x) = 5x - 2, does f^-^1(f(3)) = 3?", steps: ["f(3) = 5(3) - 2 = 13", "f^-^1(13) = (13+2)/5 = 3 OK"], answer: "Yes, f^-^1(f(3)) = 3" },
      { problem: "Find the inverse of f(x) = x + 7", steps: ["y = x + 7 -> x = y + 7", "y = x - 7", "f^-^1(x) = x - 7"], answer: "f^-^1(x) = x - 7" },
...
function rightTriangleTrigTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Pythagorean Theorem", formula: "a^2 + b^2 = c^2  (c = hypotenuse, the longest side)", explanation: "Find any missing side using the Pythagorean theorem.", tip: "Classic triples: 3-4-5, 5-12-13, 8-15-17" },
      { title: "Finding Angles", formula: "theta = sin^-^1(opp/hyp)    theta = cos^-^1(adj/hyp)    theta = tan^-^1(opp/adj)", explanation: "Use inverse trig functions when you have two sides and need an angle.", tip: "sin(30 degrees)=0.5    cos(60 degrees)=0.5    tan(45 degrees)=1" },
    ],
    examples: [
      { problem: "A right triangle has opposite=3, hypotenuse=5. Find sin(theta).", steps: ["SOH: sin = Opposite/Hypotenuse", "sin(theta) = 3/5"], answer: "3/5" },
      { problem: "A right triangle has adjacent=4, hypotenuse=5. Find cos(theta).", steps: ["CAH: cos = Adjacent/Hypotenuse", "cos(theta) = 4/5"], answer: "4/5" },
      { problem: "A right triangle has opposite=3, adjacent=4. Find tan(theta).", steps: ["TOA: tan = Opposite/Adjacent", "tan(theta) = 3/4"], answer: "3/4" },
      { problem: "Find the hypotenuse of a right triangle with legs 5 and 12.", steps: ["Pythagorean theorem: c^2 = a^2 + b^2", "c^2 = 25 + 144 = 169", "c = sqrt169 = 13"], answer: "13" },
      { problem: "A ladder 10m long leans against a wall at 30 degrees to the ground. How high does it reach?", steps: ["sin(30 degrees) = opposite/hypotenuse", "sin(30 degrees) = 0.5", "Height = 10 x sin(30 degrees) = 10 x 0.5 = 5m"], answer: "5 m" },
    ],
  };
}
...
function unitCircleTutorial(): TutorialContent {
  return {
    ...
    intro: "The unit circle has radius 1. At angle theta, the point is (cos theta, sin theta). Key angles: 0 degrees, 30 degrees, 45 degrees, 60 degrees, 90 degrees.",
    concepts: [
      { title: "Unit Circle Definition", formula: "Point at angle theta = (cos theta, sin theta) on a circle of radius 1", explanation: "cos gives the x-coordinate, sin gives the y-coordinate. The circle ties geometry and algebra together.", tip: "At theta=0 degrees: (1,0) meaning cos0=1 and sin0=0" },
      { title: "Key Angles to Memorise", formula: "0 degrees=(1,0)  30 degrees=(sqrt3/2,1/2)  45 degrees=(sqrt2/2,sqrt2/2)  60 degrees=(1/2,sqrt3/2)  90 degrees=(0,1)", explanation: "These five angles appear on every exam. Memorise their coordinates.", tip: "sin pattern for 0->90 degrees: 0, 1/2, sqrt2/2, sqrt3/2, 1" },
    ...
    examples: [
      { problem: "What is sin(0 degrees) and cos(0 degrees)?", steps: ["At 0 degrees, the point is (1, 0)", "cos(0 degrees) = 1, sin(0 degrees) = 0"], answer: "sin(0 degrees)=0, cos(0 degrees)=1" },
      { problem: "What is sin(90 degrees) and cos(90 degrees)?", steps: ["At 90 degrees, the point is (0, 1)", "cos(90 degrees) = 0, sin(90 degrees) = 1"], answer: "sin(90 degrees)=1, cos(90 degrees)=0" },
      { problem: "What is sin(45 degrees)?", steps: ["At 45 degrees, the point is (sqrt2/2, sqrt2/2)", "sin(45 degrees) = sqrt2/2 ~= 0.707"], answer: "sqrt2/2" },
      { problem: "What is cos(60 degrees)?", steps: ["At 60 degrees, the point is (1/2, sqrt3/2)", "cos(60 degrees) = 1/2"], answer: "1/2" },
      { problem: "In which quadrant is sin(theta) negative and cos(theta) positive?", steps: ["Quadrant I: both positive", "Quadrant II: sin+, cos-", "Quadrant III: both negative", "Quadrant IV: sin-, cos+"], answer: "Quadrant IV" },
    ],
  };
}
...
function trigIdentitiesTutorial(): TutorialContent {
  return {
    ...
    intro: "Trig identities are equations that are true for all angles. The most important: sin^2theta + cos^2theta = 1.",
    concepts: [
      { title: "Pythagorean Identity", formula: "sin^2theta + cos^2theta = 1", explanation: "The fundamental identity. Comes from the Pythagorean theorem applied to the unit circle.", tip: "Rearrange: sin^2theta=1-cos^2theta   or   cos^2theta=1-sin^2theta" },
      { title: "Derived Pythagorean Identities", formula: "1+tan^2theta=sec^2theta    1+cot^2theta=csc^2theta", explanation: "Divide the main identity by cos^2theta or sin^2theta to get these two variations.", tip: "Appear constantly in calculus and integration" },
      { title: "Double Angle Formulas", formula: "sin(2theta)=2sinthetacostheta    cos(2theta)=cos^2theta-sin^2theta", explanation: "Double angle formulas reduce functions of 2theta to functions of theta.", tip: "cos(2theta) has three equivalent forms -- choose the most useful" },
    ],
    examples: [
      { problem: "Complete: sin^2theta + ___ = 1", steps: ["This is the Pythagorean identity.", "sin^2theta + cos^2theta = 1"], answer: "cos^2theta" },
      { problem: "Simplify: 1 - cos^2theta", steps: ["From sin^2theta + cos^2theta = 1", "Rearrange: sin^2theta = 1 - cos^2theta"], answer: "sin^2theta" },
      { problem: "What is tan theta in terms of sin and cos?", steps: ["tan theta = sin theta / cos theta"], answer: "sin theta / cos theta" },
      { problem: "Simplify: sin^2theta / cos^2theta", steps: ["= (sin theta / cos theta)^2", "= tan^2theta"], answer: "tan^2theta" },
      { problem: "What is sin(2theta)?", steps: ["Double angle formula:", "sin(2theta) = 2 sin theta cos theta"], answer: "2 sin theta cos theta" },
    ],
  };
}
...
function logarithmsTutorial(): TutorialContent {
  return {
    ...
    intro: "log_b(x) = y means b^y = x. The logarithm asks: 'What power do I raise b to, to get x?'",
    concepts: [
      { title: "Definition", formula: "log_b(x) = y  <=>  b^y = x", explanation: "A logarithm asks: what power do I raise the base to, to get this number? It is the inverse of exponentiation.", tip: "log_2(8)=3 because 2^3=8" },
      { title: "Product Rule", formula: "log_b(MN) = log_b(M) + log_b(N)", explanation: "The log of a product equals the sum of the logs.", tip: "log(6x5) = log6 + log5" },
      ...
      { title: "Power Rule", formula: "log_b(M^n) = n . log_b(M)", explanation: "The log of a power brings the exponent down as a multiplier.", tip: "log(x^3) = 3.log(x)" },
      { title: "Special Values", formula: "log_b(1)=0    log_b(b)=1    log_b(b^n)=n", explanation: "Four key values every student must know. The log of 1 is always 0. The log of the base is always 1.", tip: "log_3(3)=1   log_5(5^4)=4   10^(log 7)=7" },
    ],
    examples: [
      { problem: "Evaluate: log_2(8)", steps: ["Ask: 2 to what power = 8?", "2^1 = 2, 2^2 = 4, 2^3 = 8", "log_2(8) = 3"], answer: "3" },
      { problem: "Evaluate: log_1_0(1000)", steps: ["Ask: 10 to what power = 1000?", "10^1 = 10, 10^2 = 100, 10^3 = 1000", "log(1000) = 3"], answer: "3" },
      { problem: "Solve: log_3(x) = 4", steps: ["log_3(x) = 4 means 3^4 = x", "3^4 = 3 x 3 x 3 x 3 = 81", "x = 81"], answer: "x = 81" },
      { problem: "Expand: log(AB)", steps: ["Product rule: log(AB) = log A + log B"], answer: "log A + log B" },
      { problem: "Simplify: log_5(5^3)", steps: ["Power rule: log_b(b^n) = n", "log_5(5^3) = 3"], answer: "3" },
    ],
  };
}
...
function exponentialFunctionsTutorial(): TutorialContent {
  return {
    ...
    intro: "Exponential functions have the form f(x) = a^x. They grow (or decay) at a constant percentage rate.",
    concepts: [
      { title: "Laws of Exponents", formula: "a^m.a^n=a^m+^n    a^m/a^n=a^m^-^n    (a^m)^n=a^m^n    a^0=1    a^-^n=1/a^n", explanation: "These five laws govern all exponential expressions. Memorise them.", tip: "2^3x2^4=2^7=128    (3^2)^3=3^6=729" },
      { title: "Growth vs Decay", formula: "base>1 -> growth    0<base<1 -> decay", explanation: "If the base is greater than 1, the function grows. If the base is between 0 and 1, it decays.", tip: "2^x grows    (0.5)^x decays" },
    ...
    examples: [
      { problem: "Evaluate: 2^4", steps: ["2^4 = 2 x 2 x 2 x 2 = 16"], answer: "16" },
      { problem: "Evaluate: 3^-^2", steps: ["Negative exponent: a^-^n = 1/a^n", "3^-^2 = 1/3^2 = 1/9"], answer: "1/9" },
      { problem: "Solve: 2^x = 32", steps: ["Write 32 as a power of 2:", "32 = 2^5", "So 2^x = 2^5 -> x = 5"], answer: "x = 5" },
      { problem: "Is f(x) = (0.5)^x growth or decay?", steps: ["Base 0.5 < 1 -> as x increases, f decreases", "This is exponential DECAY"], answer: "Exponential decay" },
      { problem: "What is the y-intercept of f(x) = 4^x + 2?", steps: ["Set x = 0: f(0) = 4^0 + 2 = 1 + 2 = 3", "y-intercept = (0, 3)"], answer: "(0, 3)" },
    ],
  };
}
...
function complexNumbersTutorial(): TutorialContent {
  return {
    ...
    intro: "Complex numbers have the form a + bi, where i = sqrt(-1) and i^2 = -1. 'a' is the real part; 'b' is the imaginary part.",
    concepts: [
      { title: "The Imaginary Unit i", formula: "i = sqrt(-1)    i^2 = -1    i^3 = -i    i^4 = 1", explanation: "We define i as the square root of -1. Powers of i cycle through 4 values.", tip: "Cycle: i, -1, -i, 1, i, -1, -i, 1..." },
      ...
      { title: "Conjugate and Modulus", formula: "conjugate of a+bi = a-bi    |a+bi| = sqrt(a^2+b^2)", explanation: "The conjugate flips the sign of the imaginary part. The modulus is the distance from origin.", tip: "Conjugate of 3+4i = 3-4i    |3+4i|=5" },
    ],
    examples: [
      { problem: "What is i^2?", steps: ["By definition: i = sqrt(-1)", "i^2 = (sqrt(-1))^2 = -1"], answer: "-1" },
      { problem: "Add: (3 + 2i) + (4 + 5i)", steps: ["Add real parts: 3 + 4 = 7", "Add imaginary parts: 2i + 5i = 7i", "Answer: 7 + 7i"], answer: "7 + 7i" },
      { problem: "Simplify: sqrt(-9)", steps: ["sqrt(-9) = sqrt(9 x -1) = sqrt9 x sqrt(-1) = 3i"], answer: "3i" },
      ...
      { problem: "Multiply: (2 + 3i)(2 - 3i)", steps: ["Use difference of squares: (a+b)(a-b) = a^2 - b^2", "= 4 - (3i)^2 = 4 - 9i^2 = 4 - 9(-1) = 4 + 9 = 13"], answer: "13" },
    ],
  };
}
...
function limitsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Direct Substitution", formula: "If f is continuous at a: lim(x->a) f(x) = f(a)", explanation: "For well-behaved functions, just substitute x=a. Use more advanced methods only when you get 0/0 or inf/inf.", tip: "lim(x->3) x^2+1 = 10 (direct substitution)" },
      { title: "0/0 Form: Factor and Cancel", formula: "If 0/0: factor, cancel the problematic term", explanation: "When direct substitution gives 0/0, factor and cancel the term causing the zero.", tip: "lim(x->2)(x^2-4)/(x-2) = lim(x+2) = 4" },
    ],
    examples: [
      { problem: "Find lim(x->3) of x + 2", steps: ["Simply substitute x = 3:", "3 + 2 = 5"], answer: "5" },
      { problem: "Find lim(x->0) of x^2", steps: ["As x -> 0: x^2 -> 0^2 = 0"], answer: "0" },
      { problem: "Find lim(x->2) of (x^2 - 4)/(x - 2)", steps: ["Direct substitution gives 0/0 (indeterminate form)", "Factor numerator: (x+2)(x-2)/(x-2)", "Cancel (x-2): lim = x + 2 as x->2 = 4"], answer: "4" },
      { problem: "Find lim(x->inf) of 1/x", steps: ["As x gets very large, 1/x gets very small", "lim(x->inf) of 1/x = 0"], answer: "0" },
...
function sequencesTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Arithmetic Sequence", formula: "a_n = a_1 + (n-1)d    d = common difference", explanation: "Add the same number (d) each step. The nth term formula jumps directly to any term.", tip: "3,7,11,15: d=4    a_1_0 = 3+9x4 = 39" },
      { title: "Geometric Sequence", formula: "a_n = a_1 x r^n^-^1    r = common ratio", explanation: "Multiply by the same number (r) each step.", tip: "2,6,18,54: r=3    a_5=2x3^4=162" },
    ...
    examples: [
      ...
      { problem: "Find the 10th term of: 2, 5, 8, 11, ...", steps: ["First term a = 2, common difference d = 3", "a_n = a + (n-1)d", "a_1_0 = 2 + (10-1)x3 = 2 + 27 = 29"], answer: "29" },
      { problem: "Find the next term: 3, 6, 12, 24, ___", steps: ["Common ratio: 6/3 = 2", "Next term: 24 x 2 = 48"], answer: "48" },
      { problem: "Find the sum of an infinite geometric series: a=2, r=1/2", steps: ["Formula: S = a/(1-r)", "S = 2/(1-0.5) = 2/0.5 = 4"], answer: "4" },
      { problem: "Find the sum of first 5 terms: 1, 3, 5, 7, 9", steps: ["Arithmetic series sum: S = n/2 x (first + last)", "S = 5/2 x (1 + 9) = 5/2 x 10 = 25"], answer: "25" },
    ],
  };
}
...
function vectorsTutorial(): TutorialContent {
  return {
    skillName: "Vectors",
    intro: "A vector has magnitude (size) and direction. Written as (x, y). Add by adding components; magnitude = sqrt(x^2 + y^2).",
    concepts: [
      ...
      { title: "Magnitude", formula: "|v| = sqrt(x^2 + y^2)  (Pythagorean theorem)", explanation: "The magnitude is the length of the vector arrow.", tip: "|(3,4)| = sqrt(9+16) = sqrt25 = 5" },
      { title: "Dot Product", formula: "u.v = u_1v_1 + u_2v_2    result = 0 -> perpendicular vectors", explanation: "Multiply matching components and add. A zero dot product means the vectors are at 90 degrees to each other.", tip: "(2,3).(4,5) = 8+15 = 23" },
    ],
    examples: [
      { problem: "Add vectors: (3, 4) + (1, 2)", steps: ["Add x components: 3 + 1 = 4", "Add y components: 4 + 2 = 6", "Result: (4, 6)"], answer: "(4, 6)" },
      { problem: "Find the magnitude of (3, 4)", steps: ["magnitude = sqrt(x^2 + y^2)", "= sqrt(9 + 16) = sqrt25 = 5"], answer: "5" },
      { problem: "Find the dot product of (2, 3) and (4, 5)", steps: ["Dot product = x_1x_2 + y_1y_2", "= 2x4 + 3x5 = 8 + 15 = 23"], answer: "23" },
      { problem: "Subtract: (5, 3) - (2, 1)", steps: ["Subtract components:", "(5-2, 3-1) = (3, 2)"], answer: "(3, 2)" },
      { problem: "If two vectors have dot product 0, what does that mean?", steps: ["Dot product = 0 means the vectors are perpendicular (90 degrees angle)"], answer: "They are perpendicular" },
    ],
  };
}
...
function derivativesTutorial(): TutorialContent {
  return {
    ...
    intro: "The derivative f'(x) measures the instantaneous rate of change. Power rule: d/dx[x^n] = nx^n^-^1.",
    concepts: [
      { title: "Power Rule", formula: "d/dx[x^n] = nx^n^-^1", explanation: "Bring the exponent down as a multiplier, then reduce the exponent by 1.", tip: "d/dx[x^5]=5x^4    d/dx[x^2]=2x    d/dx[x]=1" },
      { title: "Sum Rule", formula: "d/dx[f+/-g] = f'+/-g'", explanation: "Differentiate each term separately. The derivative distributes over addition.", tip: "d/dx[x^3+2x]=3x^2+2" },
      { title: "Key Derivatives", formula: "d/dx[e^x]=e^x    d/dx[ln x]=1/x    d/dx[sin x]=cos x    d/dx[cos x]=-sin x", explanation: "These fundamental derivatives must be memorised.", tip: "e^x is its own derivative!" },
    ],
    examples: [
      { problem: "Find d/dx[x^3]", steps: ["Power rule: bring down exponent, reduce by 1", "d/dx[x^3] = 3x^2"], answer: "3x^2" },
      { problem: "Find d/dx[5x^2]", steps: ["Constant multiple rule: multiply constant by derivative", "d/dx[5x^2] = 5 x 2x = 10x"], answer: "10x" },
      { problem: "Find d/dx[x^4 + 3x]", steps: ["Differentiate term by term:", "d/dx[x^4] = 4x^3", "d/dx[3x] = 3", "Answer: 4x^3 + 3"], answer: "4x^3 + 3" },
...
function integralsTutorial(): TutorialContent {
  return {
    skillName: "Integrals",
    intro: "Integration is the reverse of differentiation. Power rule: integralx^n dx = x^n+^1/(n+1) + C.",
    concepts: [
      { title: "Power Rule for Integration", formula: "integralx^n dx = x^n+^1/(n+1) + C    (n != -1)", explanation: "Increase the exponent by 1, then divide by the new exponent. Always add +C.", tip: "integralx^2 dx = x^3/3 + C" },
      { title: "The +C", formula: "integralf(x)dx = F(x) + C", explanation: "The indefinite integral always has +C because there are infinitely many antiderivatives.", tip: "Both x^2+5 and x^2-3 differentiate to 2x" },
      { title: "Fundamental Theorem", formula: "integral[a to b] f(x)dx = F(b) - F(a)", explanation: "To evaluate a definite integral: find the antiderivative, then substitute and subtract.", tip: "integral[0 to 3] x dx = [x^2/2] from 0 to 3 = 4.5" },
    ],
    examples: [
      { problem: "Find integralx^2 dx", steps: ["Power rule: increase exponent by 1, divide by new exponent", "integralx^2 dx = x^3/3 + C"], answer: "x^3/3 + C" },
      { problem: "Find integral4x dx", steps: ["integral4x dx = 4 x x^2/2 + C = 2x^2 + C"], answer: "2x^2 + C" },
      { problem: "Evaluate integral[0 to 2] x dx", steps: ["integralx dx = x^2/2 + C", "Evaluate from 0 to 2:", "[x^2/2] from 0 to 2 = 4/2 - 0/2 = 2"], answer: "2" },
      { problem: "Find integralcos x dx", steps: ["Standard integral: integralcos x dx = sin x + C"], answer: "sin x + C" },
      { problem: "Find integrale^x dx", steps: ["The exponential integral: integrale^x dx = e^x + C"], answer: "e^x + C" },
    ],
  };
}
...
function calculusApplicationsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Critical Points", formula: "Set f'(x)=0 and solve for x", explanation: "Critical points are where the slope is zero -- possible local maxima or minima. Always check the second derivative to classify them.", tip: "f(x)=x^2-4x: f'(x)=2x-4=0 -> x=2 is a critical point" },
      ...
      { title: "Area Under a Curve", formula: "Area = integral[a to b] f(x) dx = F(b) - F(a)", explanation: "The definite integral gives the area between the curve and x-axis from a to b.", tip: "Area under y=x from 0 to 4: [x^2/2] = 8" },
    ],

    examples: [
      { problem: "If f(x) = x^2, at what x is the slope of the tangent = 4?", steps: ["f'(x) = 2x", "Set 2x = 4: x = 2"], answer: "x = 2" },
      { problem: "Find the critical points of f(x) = x^3 - 3x", steps: ["f'(x) = 3x^2 - 3", "Set f'(x) = 0: 3x^2 - 3 = 0", "x^2 = 1 -> x = +/-1"], answer: "x = 1 and x = -1" },
      ...
      { problem: "Find the area under y = 2x from x=0 to x=3.", steps: ["integral[0 to 3] 2x dx = [x^2] from 0 to 3 = 9 - 0 = 9"], answer: "9 square units" },
      { problem: "A ball has position s(t) = -5t^2 + 20t. When is velocity = 0?", steps: ["Velocity = s'(t) = -10t + 20", "Set to 0: -10t + 20 = 0 -> t = 2 seconds"], answer: "t = 2 seconds" },
    ],
  };
}
...
function nounsTutorial(): TutorialContent {
  return {
    ...
    examples: [
      ...
      { problem: "Find the nouns: 'The dog chased the cat into the garden.'", steps: ["dog = thing OK", "cat = thing OK", "garden = place OK"], answer: "dog, cat, garden" },
...
function verbsTutorial(): TutorialContent {
  return {
    ...
    examples: [
      ...
      { problem: "What is subject-verb agreement?", steps: ["Singular subject -> singular verb: 'She runs.'", "Plural subject -> plural verb: 'They run.'", "Common error: 'She run' X -> 'She runs' OK"], answer: "Subject and verb must match in number" },
    ],
  };
}
...
function punctuationTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "End Punctuation", formula: ". statement    ? question    ! strong emotion or command", explanation: "Every sentence ends with one of three marks. Choose based on the type of sentence.", tip: "'The cat sat.' OK    'Where is it.' X -> 'Where is it?' OK" },
...
function essayTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "5-Paragraph Essay Structure", formula: "Introduction + Body (x3) + Conclusion", explanation: "The classic structure. Intro with thesis, three body paragraphs, conclusion.", tip: "Intro = set up    Body = prove it    Conclusion = wrap up" },
...
function statesOfMatterTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "State Changes", formula: "Melting: solid->liquid    Boiling: liquid->gas    Condensation: gas->liquid    Freezing: liquid->solid", explanation: "Adding heat makes particles move faster and spread apart, changing state.", tip: "Water freezes at 0 degreesC and boils at 100 degreesC" },
      { title: "Sublimation", formula: "Solid -> gas directly (no liquid stage)", explanation: "Some substances go straight from solid to gas. Example: dry ice (solid CO_2).", tip: "More heat = faster particles = more spacing = higher energy state" },
    ],
    examples: [
      ...
      { problem: "What is the difference between evaporation and boiling?", steps: ["Evaporation happens at the surface, at any temperature.", "Boiling happens throughout the liquid, at boiling point (100 degreesC for water).", "Both change liquid to gas."], answer: "Evaporation is surface/slow; boiling is throughout/fast at 100 degreesC" },
      { problem: "What is sublimation?", steps: ["Sublimation is when a solid changes DIRECTLY to a gas.", "No liquid stage in between.", "Example: dry ice (solid CO_2) disappears at room temperature."], answer: "Solid changing directly to gas (e.g., dry ice)" },
...
function lifeScienceTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Photosynthesis", formula: "6CO_2 + 6H_2O + light -> C_6H_1_2O_6 + 6O_2", explanation: "Plants use sunlight, water, and carbon dioxide to make glucose and release oxygen. Happens in chloroplasts.", tip: "Inputs: CO_2 + water + sunlight    Outputs: glucose + oxygen" },
    ...
    examples: [
      { problem: "What do plants need for photosynthesis?", steps: ["Sunlight (energy)", "Water (from soil through roots)", "Carbon dioxide (from air through stomata)", "Equation: 6CO_2 + 6H_2O + light -> C_6H_1_2O_6 + 6O_2"], answer: "Sunlight, water, carbon dioxide" },
...
function earthScienceTutorial(): TutorialContent {
  return {
    ...
    examples: [
      ...
      { problem: "What causes the seasons?", steps: ["Earth's axis is tilted at 23.5 degrees.", "As Earth orbits the Sun, different hemispheres tilt towards the Sun.", "Tilted towards Sun = summer (more direct sunlight).", "Tilted away = winter."], answer: "Earth's tilted axis as it orbits the Sun" },
...
function physicsTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      { title: "Newton's Three Laws", formula: "1st: inertia    2nd: F=ma    3rd: action=reaction", explanation: "Law 1: objects keep their motion unless a force acts. Law 2: force = mass x acceleration. Law 3: every action has equal and opposite reaction.", tip: "Seatbelt (1st)    Push heavier box = more force (2nd)    Rocket exhaust (3rd)" },
      ...
      { title: "Energy Types", formula: "Kinetic: KE=1/2mv^2    Potential: PE=mgh    Conservation: total energy stays constant", explanation: "Energy cannot be created or destroyed, only converted. KE and PE constantly trade off.", tip: "Ball at top: max PE, zero KE    At bottom: max KE, zero PE" },
    ],
    examples: [
      ...
      { problem: "What is Newton's Second Law (F = ma)?", steps: ["Force = mass x acceleration", "A bigger force -> greater acceleration", "A heavier object needs more force to accelerate the same amount.", "Example: pushing a car vs pushing a bicycle"], answer: "F = ma (force = mass x acceleration)" },
...
function chemistryTutorial(): TutorialContent {
  return {
    ...
    concepts: [
      ...
      { title: "Chemical Bonds", formula: "Ionic = transfer electrons (metal+non-metal)    Covalent = share electrons (non-metal+non-metal)", explanation: "Ionic bonds form when atoms transfer electrons. Covalent bonds form when atoms share electrons.", tip: "NaCl (salt) = ionic    H_2O (water) = covalent" },
    ...
    examples: [
      ...
      { problem: "What is the difference between an ionic and covalent bond?", steps: ["Ionic bond: electrons are TRANSFERRED from one atom to another", "Example: NaCl (table salt) -- Na gives electron to Cl", "Covalent bond: electrons are SHARED between atoms", "Example: H_2O -- oxygen and hydrogen share electrons"], answer: "Ionic = transfer electrons; Covalent = share electrons" },
