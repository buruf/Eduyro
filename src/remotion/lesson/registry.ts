...
import { factorLines } from "./script-factor";
import { QUAD_UNITS } from "./units-quad";
import { quadLines } from "./script-quad";
...
import { polyOpsLines } from "./script-polyops";
...
export const ALL_LESSON_UNITS: RegisteredUnit[] = [
  ...
  ...FACTOR_UNITS.map((u) => ({ id: u.id, label: u.label, comp: "Factor", lines: () => factorLines(u) })),
  ...QUAD_UNITS.map((u) => ({ id: u.id, label: u.label, comp: "Quad", lines: () => quadLines(u) })),
