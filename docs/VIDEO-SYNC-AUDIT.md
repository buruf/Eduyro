...
| RatioTable | 22 | ✔ Sep 12 — table cells, scale arrows and new counters land on their number in narration order; counters already on screen stay put; 3 units verified by stills. Before: the whole ×2 column was up before "times 2", the record equation at 0 s |
...
| PolyOps | 11 | ✔ Sep 13 — all 8 modes: terms, cards, substitution rows, room bodies, division rows and GCF factors land on their number in narration order; 3 modes verified by stills. Before: the whole headline and all three cards at 0 s. Note: three record scenes carry no spoken numbers and stay on fractions by design |
...
| PreAlg | 7 | ✔ Sep 13 — the value pours into the x-box on "x is 4", each counted group lands on its running total, the rectangle cuts on "a piece of length 4" and each room labels on its product, the balance's solution on "is 3", and the integer walker stands on every number of the walk; 6 units verified by stills (142). Before: the whole expression, both rooms and the answer were on screen at 0 s and the walk was a smooth interpolation ignoring the count. Follow-up: the evaluate-add RECORD line talks about x − 7 = −3 while the picture still shows x + 7 = 11 (content mismatch, out of scope) |
...
| FunctionMachine | 5 | ✔ Sep 13 — all 6 modes: the chip drops on its input word, the output chip and its table row land on the result, a composition's inner result lands before the outer, the inverse's two undo steps land on their own numbers, and the rational machine now JAMS on "divided by zero" instead of being red from frame 0; 5 units verified by stills. Before: the whole in/out table (answers included) was on screen at 0 s. Follow-up: the mid-travel chip rests over the machine's rule text (pre-existing chip path) |
| DecimalOps | 5 | ✔ Sep 13 — shaded cells and slices land on their count, number-line ticks and the marker on their value, shares and groups on the quotient, decimals on the digits the recording carries; 6 units verified by stills. Before: the rounding marker was up 5.5 s before its value and both compare squares at once |
| Trig | 4 | ✔ Sep 13 — all 8 modes: side labels land on their length, each leg's square fills to its value, the hypotenuse arrives on "which is 5", the three side NAMES land with their number, each ratio row on the value it ends with, the compass radius walks to each point as its degrees are named, and the radian arc reaches 3 and π on "3" and "180"; 5 units verified by stills. Before: the whole headline "9 + 16 = 25" was up 6.5 s early and all three names were on screen 9 s before she named the hypotenuse. Also fixed: the answer 5 was labelled on the triangle during "How long is the slant?" |
| Quad | 4 | |
| Poly | 4 | |
| LinEq | 4 | |
| Factor | 4 | |
